...
import { Server, Activity, Shield, Settings, Plus, Minus, Eye } from 'lucide-react'
...
function Kubernetes() {
  ...
  const scaleDeployment = async (deploymentId: any, replicas: any) => {
    try {
      const { default: API_CONFIG } = await import('../api/config')
  ...
  const __getResourceColor = (percentage: any) => {
              ...
              <div key={index} className="mb-4 p-3 bg-gray-900 rounded-lg border border-gray-700">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <h3 className="text-white font-medium">{(cluster as any).name}</h3>
                    <p className="text-gray-400 text-sm">{(cluster as any).version} • {(cluster as any).provider}</p>
                    <div className="flex items-center mt-2 space-x-4">
                      <span className={`px-2 py-1 rounded text-xs ${getStatusColor((cluster as any).status)}`}>
                        {(cluster as any).status}
                      </span>
                      <span className="text-xs text-gray-400">
                        {(cluster as any).nodes?.length || 0} nodes
                      </span>
                      <span className="text-xs text-gray-400">
                        {(cluster as any).region}
                      </span>
                    </div>
                    <div className="mt-2">
                      ...
                      <div className="flex space-x-4 mt-1">
                        <span className="text-xs text-gray-300">
                          CPU: {(cluster as any).resources?.used_cpu || '0'}/{(cluster as any).resources?.total_cpu || '0'}
                        </span>
                        <span className="text-xs text-gray-300">
                          Memory: {(cluster as any).resources?.used_memory || '0'}/{(cluster as any).resources?.total_memory || '0'}
                        </span>
                      </div>
                    </div>
                  </div>
          ...
          <div className="p-4 max-h-96 overflow-y-auto">
            {Object.values(deployments).map((deployment, index) => (
              <div key={index} className="mb-4 p-3 bg-gray-900 rounded-lg border border-gray-700">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <h3 className="text-white font-medium">{(deployment as any).name}</h3>
                    <p className="text-gray-400 text-sm">{(deployment as any).image}</p>
                    <div className="flex items-center mt-2 space-x-4">
                      <span className={`px-2 py-1 rounded text-xs ${getStatusColor((deployment as any).status)}`}>
                        {(deployment as any).status}
                      </span>
                      <span className="text-xs text-gray-400">
                        {(deployment as any).ready_replicas}/{(deployment as any).replicas} ready
                      </span>
                      <span className="text-xs text-gray-400">
                    ...
                    <div className="mt-2">
                      ...
                      <div className="flex space-x-4 mt-1">
                        <span className="text-xs text-gray-300">
                          CPU: {(deployment as any).resources?.requests?.cpu || 'N/A'}
                        </span>
                        <span className="text-xs text-gray-300">
                          Memory: {(deployment as any).resources?.requests?.memory || 'N/A'}
                        </span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2">
                    <button 
                      onClick={() => scaleDeployment((deployment as any).id, (deployment as any).replicas + 1)}
                    ...
                    <button 
                      onClick={() => scaleDeployment((deployment as any).id, Math.max(1, (deployment as any).replicas - 1))}
          ...
          <div className="p-4 max-h-96 overflow-y-auto">
            {Object.values(services).map((service, index) => (
              <div key={index} className="mb-4 p-3 bg-gray-900 rounded-lg border border-gray-700">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-white font-medium">{(service as any).name}</h3>
                    <p className="text-gray-400 text-sm">{(service as any).type}</p>
                    <div className="flex items-center mt-2 space-x-4">
                      <span className={`px-2 py-1 rounded text-xs ${getStatusColor((service as any).status)}`}>
                        {(service as any).status}
                      </span>
                      <span className="text-xs text-gray-400">
                        {(service as any).cluster_ip}
                      </span>
                      <span className="text-xs text-gray-400">
                        {(service as any).ports?.length || 0} ports
                      </span>
                    </div>
                    <div className="mt-2">
                      <p className="text-xs text-gray-400">Ports:</p>
                      <div className="flex space-x-2 mt-1">
                        {(service as any).ports?.map((port, i) => (
          ...
          <div className="p-4 max-h-96 overflow-y-auto">
            {Object.values(autoscalers).map((autoscaler, index) => (
              <div key={index} className="mb-4 p-3 bg-gray-900 rounded-lg border border-gray-700">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-white font-medium">{(autoscaler as any).name}</h3>
                    <p className="text-gray-400 text-sm">Target: {(autoscaler as any).target_deployment}</p>
                    <div className="flex items-center mt-2 space-x-4">
                      <span className={`px-2 py-1 rounded text-xs ${getStatusColor((autoscaler as any).status)}`}>
                        {(autoscaler as any).status}
                      </span>
                      <span className="text-xs text-gray-400">
                        {(autoscaler as any).current_replicas}/{(autoscaler as any).max_replicas}
                      </span>
                      <span className="text-xs text-gray-400">
                        Min: {(autoscaler as any).min_replicas}
                      </span>
                    </div>
                    <div className="mt-2">
                      ...
                      <div className="flex space-x-4 mt-1">
                        <span className="text-xs text-gray-300">
                          CPU: {(autoscaler as any).target_cpu_utilization}%
                        </span>
                        <span className="text-xs text-gray-300">
                          Memory: {(autoscaler as any).target_memory_utilization}%
                        </span>
                      </div>
                    </div>
                  </div>
