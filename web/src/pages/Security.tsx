...
function Security() {
  ...
  const { policies, vulnerabilities, securityScore, loading, error, refreshData } = securityData
            ...
            <tbody>
              {policies?.map((policy) => (
                <tr key={policy.id} className="border-b border-gray-800">
                  <td className="py-3 px-4 text-white">{sanitizeInput(policy.name)}</td>
          ...
          {vulnerabilities?.map((vuln) => (
            <div key={vuln.id} className={`p-4 rounded-lg border ${getSeverityColor(vuln.severity)}`}>
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-white font-medium">{sanitizeInput(vuln.name || vuln.title)}</h3>
                  <p className="text-gray-400 text-sm mt-1">Affected: {sanitizeInput(vuln.affected_systems?.join(', ') || vuln.affected)}</p>
                </div>
