...
import { AlertTriangle, PlayCircle, PauseCircle, CheckCircle, Clock, Users, FileText, Settings, Eye } from 'lucide-react'

function IncidentResponse({ user }) {
  ...
  const executePlaybook = async (incidentId, playbookId) => {
    try {
      const { default: API_CONFIG } = await import('../api/config')
  ...
  const pauseResponse = async (responseId) => {
    try {
      const { default: API_CONFIG } = await import('../api/config')
