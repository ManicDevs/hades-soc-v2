...
export const threatModelingAPI = {
  ...
  // Create threat model
  createModel: async (modelData: Record<string, any>) => {
    return await API_CONFIG.request('/threat/models', {
      method: 'POST',
  ...
  createScenario: async (scenarioData: Record<string, any>) => {
    return await API_CONFIG.request('/threat/scenarios', {
      method: 'POST',
  ...
  updateVulnerability: async (vulnId: string, status: string) => {
  ...
  getSimulationResults: async (simulationId: string) => {
    return await API_CONFIG.request(`/threat/simulations/${simulationId}/results`)
  }
}
