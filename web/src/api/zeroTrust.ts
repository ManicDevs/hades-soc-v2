...
export const zeroTrustAPI = {
  ...
  getNetworkSegments: async (): Promise<unknown> => {
    return await API_CONFIG.request("/zerotrust/network-segments");
  },

  // Update policy
  updatePolicy: async (policyId: string, status: string): Promise<unknown> => {
  ...
  // Create policy
  createPolicy: async (policyData: Record<string, unknown>) => {
