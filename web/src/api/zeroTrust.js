...
export const zeroTrustAPI = {
  ...
  getNetworkSegments: async () => {
    return await API_CONFIG.request('/zerotrust/network-segments')
  },

  // Update policy
  updatePolicy: async (policyId, status) => {
  ...
  processAccessRequest: async (requestId, decision) => {
    return await API_CONFIG.request('/zerotrust/access-process', {
      method: 'POST',
